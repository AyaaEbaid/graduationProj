...
const CraftsmanProfile = () => {
  ...
  const prevImage = () =>
    setSelectedImageIndex((prev) => (prev === 0 ? workerData.cataloge.length - 1 : prev - 1));
  const nextImage = () =>
    setSelectedImageIndex((prev) => (prev === workerData.cataloge.length - 1 ? 0 : prev + 1));
  ...
  if (errorProfile || error)
    return <div  className="text-center p-6">{t(`header.craftsmanProfile.${errorProfile || error}`)}</div>;
  if (!workerData) return <div className="text-center p-6">{t("header.craftsmanProfile.noData")}</div>;
            ...
            <div className="flex flex-col items-center">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full">
                {workerData.cataloge && workerData.cataloge.length > 0 ? (
                  ...
                  <div className="col-span-2 sm:col-span-3 w-full h-48 flex items-center justify-center bg-gray-200 text-teal-600 text-lg font-semibold text-center">
                    {t("header.craftsmanProfile.noCatalog")}
                  </div>
                )}
              </div>
        ...
        >
          <div className="relative">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">{t("header.craftsmanProfile.email")}</label>
                <input
                  type="email"
              ...
              <div>
                ...
                <select
                  value={editedData?.centerId || 0}
                  onChange={(e) => setEditedData({ ...editedData, centerId: parseInt(e.target.value) || 0 })}
                  className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:ring-teal-500 focus:border-teal-500"
                  disabled={centers.length === 0}
                >
                  <option value={0}>{i18n.language === "ar" ? "اختر مركز" : "Select Center"}</option>
