...
export default function WorkersList3() {
  ...
  useEffect(() => {
    const fetchGovernorates = async () => {
      ...
      try {
        ...
        setError(err.message || t("workerList3.failedToLoadGovernorates"));
      } finally {
        setLoading(false);
      }
    };
    ...
    const fetchCenters = async () => {
      ...
      try {
        ...
        if (centerData.length === 0) {
          setError(t("workerList3.noCentersForGovernorate"));
        }
        setCenters(centerData);
      } catch (err) {
        setError(err.message || t("workerList3.failedToLoadCenters"));
      } finally {
        setLoading(false);
      }
    };
      ...
      try {
        ...
        for (const worker of workers) {
          ...
          const isValidImageUrl =
            ...
            imageUrl.startsWith("/") &&
            !imageUrl.includes("no image available");
        ...
        setError(err.message || t("workerList3.failedToFetchWorkers"));
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-5xl min-h-screen mx-auto p-4">
      <h2 className="text-2xl font-bold text-center mb-6">
        {t("workerList3.availableWorkerForEPlumbingService")}
      </h2>
...
>
        <div>
          ...
          >
            <option value="">{t("workerList3.select_governorate")}</option>
        ...
        <div>
          ...
          >
            <option value="">{t("workerList3.select_center")}</option>
            {centers.map((center) => (
      ...
      {error && (
        <p className="text-center text-red-500">
          {t("workerList3.error")}: {error}
        </p>
      )}
      {!loading && !error && filteredWorkers.length === 0 && (
        <p className="text-center">{t("workerList3.no_workers_found")}</p>
      )}
          ...
          {filteredWorkers.map((worker) => (
            ...
            >
              ...
              <p className="text-gray-500">
                {worker.specialization || t("workerList3.not_specified")}
              </p>
              <p className="text-gray-400 text-sm">
                {worker.governorate} - {worker.center}
              </p>
              <p className="text-gray-400 text-sm">
                {t("workerList3.experience")}: {worker.experience || 0} {t("workerList3.years")}
              </p>
              <button
                onClick={(e) => {
                  e.stopPropagation();
      ...
      {totalPages > 1 && (
        <div className="mt-6">
          ...
          <p className="text-center text-gray-500 mt-2">
            {t("workerList3.showing")} {filteredWorkers.length} {t("workerList3.of")} {totalRecords} {t("workerList3.workers")}
          </p>
        </div>
      )}
    </div>
