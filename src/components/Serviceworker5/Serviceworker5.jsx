...
export default function WorkersList5() {
  ...
  useEffect(() => {
    const fetchGovernorates = async () => {
      ...
      try {
        ...
        setError(err.message || t("workerList5.failedToLoadGovernorates"));
      } finally {
        setLoading(false);
      }
    };
    ...
    const fetchCenters = async () => {
      ...
      try {
        ...
        if (centerData.length === 0) {
          setError(t("workerList5.noCentersForGovernorate"));
        }
        setCenters(centerData);
      } catch (err) {
        setError(err.message || t("workerList5.failedToLoadCenters"));
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchWorkers = async () => {
      ...
      try {
        const response = await axios.get(
          `https://hanshatabhalak.runasp.net/api/Craftsman/GetCraftsmenPaginated?specializationId=${5}&centerId=${selectedCenter}&PageNumber=${currentPage}&PageSize=${itemsPerPage}&language=${i18n.language}`
        );
        ...
        for (const worker of workers) {
          ...
          const isValidImageUrl =
            imageUrl && typeof imageUrl === "string" && imageUrl.startsWith("/") && !imageUrl.includes("no image available");
        ...
        setTotalPages(Math.ceil(totalRecordsFromApi / itemsPerPage) || 1);
      } catch (err) {
        setError(err.message || t("workerList5.failedToFetchWorkers"));
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (selectedGovernorate) {
      ...
      filtered = gov ? filtered.filter((w) => w.governorate === gov.name) : [];
    }

    if (selectedCenter) {
      const center = centers.find((c) => c.id === parseInt(selectedCenter));
      filtered = center ? filtered.filter((w) => w.center === center.name) : [];
    }

    setFilteredWorkers(filtered);
  }, [workersData, selectedGovernorate, selectedCenter, governorates, centers]);

  const handleCardClick = (workerId) => navigate(`/workerportfolio/${workerId}`);
    ...
    <div className="max-w-5xl min-h-screen mx-auto p-4">
      ...
      <div className="flex justify-center space-x-4 mb-6">
        <div>
          ...
          >
            <option value="">{t("workerList5.select_governorate")}</option>
        ...
        <div>
          ...
          >
            <option value="">{t("workerList5.select_center")}</option>
            {centers.map((center) => (
      ...
      {error && (
        <p className="text-center text-red-500">
          {t("workerList5.error")}: {error}
        </p>
      )}
      {!loading && !error && filteredWorkers.length === 0 && (
        <p className="text-center">{t("workerList5.no_workers_found")}</p>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredWorkers.map((worker) => (
            ...
            >
              ...
              <p className="text-gray-500">{worker.specialization || t("workerList5.not_specified")}</p>
              <p className="text-gray-400 text-sm">
                {worker.governorate} - {worker.center}
              </p>
              <p className="text-gray-400 text-sm">
                {t("workerList5.experience")}: {worker.experience || 0} {t("workerList5.years")}
              </p>
              <button
                onClick={(e) => {
                  e.stopPropagation();
      ...
      {totalPages > 1 && (
        <div className="mt-6">
          ...
          <p className="text-center text-gray-500 mt-2">
            {t("workerList5.showing")} {filteredWorkers.length} {t("workerList5.of")} {totalRecords} {t("workerList5.workers")}
          </p>
        </div>
      )}
    </div>
